...
describe('useFetchMoviesByGenre', () => {
  const movieList: unknown[] = []
  const setMovieList = jest.fn()

  beforeEach(() => {
    jest.mocked(useEffect).mockImplementation((callback) => callback())

    jest.mocked(useState).mockReturnValue([movieList, setMovieList])
  })

  it('Ensures that the hook useFetchMoviesByGenre get and returns the movie list correctly', async () => {
    const fakeMovieGenre = 123
    const page = 10

    jest.mocked(apiService.fetchMoviesByGenre).mockResolvedValueOnce(moviesList)

    const response = useFetchMoviesByGenre(fakeMovieGenre, page)

    await waitFor(() => expect(setMovieList).toHaveBeenCalledWith(moviesList))
    await waitFor(() => expect(response).toEqual(movieList))
  })
})
